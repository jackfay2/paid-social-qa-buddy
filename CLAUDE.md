...
- **⚠️ OIDC AUDIENCE MUST MATCH THE LISTENER — this broke the @-mention on 2026-06-01.** A Cloud Run service answers to TWO URL aliases: `…-637315940254.us-west1.run.app` (project-number form) and `…-kkih6nvcjq-uw.a.run.app` (older hash form). `gcloud run services describe --format='value(status.url)'` can return *either* alias depending on the read, so NEVER source the worker's `QA_CLOUD_TASKS_OIDC_AUDIENCE` from a fresh `status.url` lookup. The audience the worker expects MUST byte-match the audience the listener signs into the token — which is the listener's `SOCIAL_WORKER_AUDIENCE` env = **`https://qa-buddy-worker-social-test-637315940254.us-west1.run.app`**. If they differ, the worker 401s every Cloud Tasks delivery (queue retries, dispatchCount climbs, listener acks "QA request received…" but no summary ever posts). When redeploying the worker, always pin `QA_CLOUD_TASKS_OIDC_AUDIENCE` to the listener's `SOCIAL_WORKER_AUDIENCE`, not to a `status.url` read. (The deploy script's habit of `URL=$(… status.url)` then `--oidc-token-audience=$URL` is the trap.)

## 🔬 Adversarial audit + fixes (2026-06-02)

Ran a multi-agent ultracode workflow auditing all 8 subsystems for correctness bugs; every candidate got an independent adversarial verifier that read the real code before it was accepted. **13 candidates → 8 confirmed → 7 fixed** (commit `62475f9`; #8 deferred as field-blocked). 401 tests green (382 worker + 19 listener); regression test per fix. Live on worker revision `00014-kzj` and verified (real Cloud Task delivered + `orchestration_completed`, so the OIDC change below didn't break auth).

- **#1 (HIGH) `"UK"` wrong-Fixed vs BQ `"GB"`** — `_canonical_country` returned 2-letter codes verbatim *before* the alias map, so the `uk→GB` entry was dead code → wrong Fix on a correct UK campaign. Now consults the alias map first.
- **#2 (HIGH) read/write header detection could diverge** — read keyed on `check_id`+`builder_input`, write independently on `pass_or_fix`+`action`; a banner/preamble row matching one side split them → verdicts to wrong cells, silently. Fixed: one **canonical header** requiring all four columns, used by both read and write (`sheets/parser.py`).
- **#3 (SECURITY) OIDC fail-closed** — `verify_oauth2_token(audience=audience or None)` meant an empty configured audience SKIPPED audience verification (accept any Google-signed token); empty SA-email skipped that check too. Now `task_auth.py` refuses when `auth_required` but either is unconfigured. (Prod was already safe — both env vars are set — but the code now enforces it.)
...
## 🦚 Peacock MODE — Phase A complete + verified live (2026-06-03)

Per Kerri (2026-06-03): Peacock is a first-class **mode**, triggered explicitly, with Peacock-specific vocabulary. Live on worker rev **`00026-2dr`** (+ naming automation + legacy-Yes/No guard; recent history: `00023-4np` enrichment, `00024-7rn` deterministic spelling, `00025-rpr` naming automation), verified end-to-end (`/readyz` 200).
- **Naming automation (commit `0e5eb2c`, rev `00025-rpr`): Kerri approved option #1 (2026-06-04)** — builders enter the expected name / key components (comma-separated) in the sheet; the bot confirms every ad set / ad name CONTAINS them. Boundary-aware (collapse non-alphanumerics → spaces, match on token boundaries: "ACQ" matches `Peacock_FBIG_ACQ` but not `ACQUIRED`). `adset_name_conventions` + `ad_name_conventions` moved from manual → real registry checks; blank input → N/A. Verified live (Pass when all names contain the components, Fix on the offender). **Only manual-by-design rows left: `download_changes` + `ad_creative_dimensions` (standard only; Peacock automates dims via Frame_Size).** Demo sheet naming rows set to Pass (adset="Peacock, FBIG", ad="eng"). **Transition guard (commit `4ab46cb`, rev `00026-2dr`):** a legacy `Yes`/`No` in a naming cell (old manual placeholder) → Review-with-guidance, never a false Fix. **Comms TODO (Jack):** ask Carrie to update the template's two naming-row guidance from "Yes/No" → "expected name / key components (comma-separated)". No per-client config needed (builder supplies the expectation per campaign — Kerri's option #1).
- **Gemini determinism fix (commit `f119988`, rev `00024-7rn`):** spelling checks were non-deterministic — same copy flipped Pass↔Review across 5 runs (measured). Pinned `temperature=0` + `topP=1` (greedy decoding) in the generation config → re-verified 5/5 identical. Removes the inconsistency AND the spurious Reviews (clean copy now stably Passes). Variance was always Pass↔Review (never a false Fix). Regression test in `test_gemini_client.py`.
- **Test coverage 93% (457 tests):** added `tests/test_wiring.py` (DI graph 50%→97%: Peacock routing, config threading, peacock_client_ids). Run `pytest --cov=app` to check. Remaining low-coverage is live-only (BQ query exec, Firestore) — exercised by real validation, not unit tests. Pre-prod still needs a live @-mention smoke (runbook).
- **MVP scope LOCKED (2026-06-04): Kerri signed off the check_id list.** Bot handles all **33** check_ids; both templates (standard 1b8hp0 + Peacock 1KWk) reconcile with **0 unrecognized**. Canonical reference: `docs/check_coverage_map.md` (~18 deterministic today, 7 data-gated/auto-upgrade via Airbyte, 4 manual, 2 conditional, 2 Peacock-mode). Standard engine validated on 5 real clients (0 bugs): `docs/standard_client_coverage_findings.md`. The data-gated 7 are the Riley/Nikki Airbyte field-sync gap (deferred; `promoted_object`/conversion_event missing for ALL clients = top priority when they have bandwidth). Direct Meta API is OFF the table; Peacock GCP data moving off by ~end of summer (adapter repoints when destination known).
- **Review enrichment (commit `9d2760e`, rev `00023-4np`):** every Review/manual row surfaces known context (data-gated Reviews append "Known: objective X, optimization goal Y"; manual naming appends actual names; manual dimensions appends ad count) — serves "max information per QA row."
- **NOT going to prod yet** (Jack's call). Prod social infra (`qa-buddy-worker-social` + `qa-buddy-runs-social`) not created; runbook ready at `docs/prod_cutover_runbook.md`.
- **URL domain-mode (2026-06-04, commit `160c475`, rev `00022-nlh`):** `ad_destination_url` now compares by HOST when the builder enters a bare domain (`peacocktv.com`) — Peacock's per-creative tracking URLs are all unique but share one domain, so full-URL never matched. Domain mode Passes cleanly + still catches an off-domain ad; full-URL input keeps exact compare. Demo sheet `ad_destination_url`=`peacocktv.com` → Pass. **Clean Peacock demo run now: Pass 9 | Fix 0 | Review 15 | N/A 3 | Error 0** (demo-sheet fills: audiences=Yes, placements set, ad_status=Paused, ad_destination_url=peacocktv.com).
- **Data-source ceiling CONFIRMED (2026-06-04):** checked perf, trafficking, raw Meta staging (`prod_peacock_social_data.src_meta`/`stg_meta`), AND `NBCU_Audience_Creative_Mapping.PaidSocial_Audience_Mapping` (only decodes funnel-stage + objective, not demographics). age/gender/geo/budget/bid/conversion-event/optimization/attribution/spend-caps are **nowhere in Peacock BQ** → Lever 2 (Meta API) is the only path. Peacock CTA vocabulary on Meta: Sign Up (dominant), Watch More, Learn More, Get Offer, NA — all recognized by `_canonical_cta` (NA→Review).
- **Parser fix (2026-06-04, commit `6b41951`, rev `00021-qnq`):** the multi-section Meta template restarts each section (Campaign/Ad Set/Ad) with its own `Check_ID` header row; `parse_check_rows` detected only the first and parsed rows 16 & 35 ("Check_ID") as bogus checks → **2 spurious `Error: Unrecognized`** in every Peacock/standard run, with "Error" written onto the header rows. Fixed: skip any row whose check_id normalizes to a Check_ID header token (`_CHECK_ID_HEADER_TOKENS`). Live sheet: 29→27 rows, 0 bogus. Cleared the stale Error cells on the demo sheet. A clean Peacock re-run is now **Pass 5 | Fix 0 | Review 17 | N/A 5 | Error 0**. Demo sheets: standard `1ORSm…` (IBD), Peacock `1KWk…`.
- **Lever 1 SHIPPED + DEPLOYED (2026-06-04, commit `98300f0`, rev `00020-gvf`):** wired every check Peacock's BigQuery data supports. `PeacockMetaClient.get_ad_sets` now aggregates per-creative `AudienceName` → `targeting.custom_audiences`, `AirTable_Placement` → `placements`, and trafficking flight dates → `start_time`/`end_time`. New/wired checks: **adset_audiences** (data-driven presence), **adset_placements** (NEW, Pass-on-exact-set-else-Review), **ad_flight_window** (NEW QC surface of `Live_After_End_Date_Warning`, ALWAYS_RUN; caught a real "Approaching End Date" live), **adset_start/end_date** (Peacock = Pass-on-match-else-Review, never false Fix). All cardinal-safe. Coverage map: `docs/peacock_coverage_plan.md`. **Demo sheet measured: Review 15→14, Pass 3→5** (audiences + placements flipped). **The deep dive PROVED the remaining ~10 settings Reviews (budget/bid/conversion-event/spend-caps/age/gender/geo/exclusions/optimization/attribution) are NOT in any Peacock BQ table** — only the Meta Marketing API has them → **Lever 2**.
- **Lever 2 (open decision):** a direct Meta Marketing API connector for the Peacock ad account (`act_172945950683253` etc.) is the ONLY way to make the settings rows deterministic. Open question for the team: does Wpromote already have Meta API access to Peacock's account?
- **Trigger (3 ways, all → route to `C22848672` = Peacock):** (a) the word **"Peacock" anywhere in the @-mention** — the worker reads it from `requester_text` (the raw text the listener already forwards), so **NO listener change was needed**; (b) an explicit `peacock:true` envelope flag; (c) a Peacock account_id that resolves to `C22848672` (e.g. `172945950683253`). The explicit trigger/flag **bypasses account resolution** (orchestration forces the Peacock client) — for current Peacock campaigns whose account may not resolve.
- **Peacock vocabulary (Kerri approved):** when `peacock_mode` (client_id is Peacock), `campaign_objective` + `campaign_buying_type` do a **direct normalized compare** in Peacock's own terms (Acquisition / Awareness / Engagement / Other; Biddable) — no Meta-enum mapping → real **Pass/Fix** instead of "could not interpret". `_peacock_value_match` in `meta_checks.py`; flag set in orchestration `_fetch_evidence` from `peacock_client_ids` (wiring: `qa_peacock_client_id`). Standard clients unchanged. (status/CTA already handled Peacock's Live/Paused, Sign Up/… via existing synonyms.)
- **Verified live:** objective→Pass, buy-type→Pass on the Peacock campaign; `peacock:true` + IBD account → forced `C22848672`; "Peacock" in text + IBD account + no flag → forced `C22848672`. Worker reads `nbc-287716` (SA grant from Pamela). 404 worker tests green.
- **Phase B v1 SHIPPED + DEPLOYED (2026-06-03, commit `64f73e8`, worker rev `00019-6x2`):** the **trafficking table** `AirTable_v2.wp_live_trafficking` is now merged into Peacock evidence. SA reads it live (74,162 rows via the project-level dataViewer grant — confirmed by impersonation). **Join (Pamela 2026-06-03):** perf `DistributionID` ↔ trafficking `Distribution_` (CAST both to STRING), with `VersionNumber` ↔ `Version_` as the fallback for un-reused historical creatives. **Verified 1076/1076 ads matched** on a live campaign. `PeacockMetaClient` does one extra (best-effort, graceful-degrade) query → attaches `ad["trafficking"]` {frame_size, asset_type, flight_start/end_date, flight_window_flag, trafficking_status, offer/show/genre}. **`ad_creative_dimensions` is now deterministic in Peacock mode** (was manual Review) — aspect-ratio compare of builder dims vs trafficked `Frame_Size`; Fix only when all sizes seen, else Review (sizes always echoed). Pipeline routes it via `PEACOCK_DETERMINISTIC_CHECK_IDS`. Worker SA reads `AirTable_v2` via the **project-level** dataViewer grant on `nbc-287716` (no new ask to Pamela). Spec: `docs/peacock_phase_b_spec.md`. **Pamela caveat:** she's *no longer maintaining the perf table* — it's our campaign_id→DistributionID bridge, current today; monitor freshness (`MAX(Date)`), pivot to trafficking-primary if it goes stale.
- **Phase B v2 (deferred, data already merged):** flight-date + QC-flag (`Live_After_End_Date_Warning`) + Offer/Show/Genre checks — wait on Kerri's semantics (do flight dates map to template Start/End; which bonus checks she wants) + the ad-vs-ad-set level question. Fields are in `ad["trafficking"]` already, so wiring is small. **Not a Pamela question** — these are Kerri's, fold into a normal check-in.
...
- ✗/⚠️ **Gemini text checks validated on REAL ad copy (2026-06-01).** Ran the live `GeminiClient` against real `facebook_ads__ad_creatives` copy: clean copy → Pass (conf 1.0; correctly does NOT flag brand terms like "IBD" or abbreviations), a typo control → Fix with specifics ("'Recieve' should be 'Receive'…"). The narrow-spelling design works. **Two bugs found:** (1) FIXED — default model `gemini-2.0-flash` is deprecated (404 "no longer available") → updated to `gemini-2.5-flash` in `app/config.py`, `app/adapters/gemini/client.py`, and the deployed worker's `QA_GEMINI_MODEL`. Without this every Gemini check fail-safes to Review. (2) ⚠️ OPEN ADAPTER GAP — ad copy lives in a **separate table** `facebook_ads__ad_creatives` (`body`=copy, `title`=headline; `object_story_spec` was NULL for C61854560), NOT on the ad record. So `BigQueryMetaClient.get_ads` doesn't fetch the copy, and the `TEXT_CHECK_DEFINITIONS` field paths (`creative.object_story_spec.link_data.message/name/description`) don't match this shape. **To make Gemini checks live:** join `facebook_ads__ad_creatives` (via `ad.ad_creative_id`) into the ad evidence and map the text-check `ad_field`s to `body`/`title`/`description` (keep the object_story_spec path as a fallback for clients that use it).
...
**Status 2026-06-16 (end of day):** Dress rehearsal PASSED live on Avara via the full Slack path (rev `00027-hc9`): `Pass 9 | Fix 2 | Review 9 | N/A 7 | Error 0`, both Fixes real catches (ad-set start dates vary; an ad isn't Active). Determinism reconfirmed: a re-run after resetting the template to pristine gave `Pass 2 | Review 11 | N/A 14` — correct for an empty sheet (the output is a function of the sheet's expected values), not a bug. New today: `docs/project_handoff.docx`/`.md` (teammate onboarding), the user-level **`google-doc` skill** (markdown → clean Google-Doc .docx; default proportional font, `--mono-code` for code-heavy docs), and `scripts/screen_am_sheet.py` (one-command AM-sheet screen: pulls account/campaign from the in-sheet link + checks synced-or-not). AM pilot guide is now `docs/qa_buddy_pilot_guide.md` (`.docx` rendered); `am_usage_guide.md` retired. **Tomorrow:** the marts `prj-npd-plrs-tst-marts-onfd` goes live → run `python scripts/verify_new_marts.py` for KS (`955066737173306`) and ZAGG (`863237077344182`); if it reports a config swap, repoint `BQ_META_PROJECT` + re-validate. **Pending (people/external):** Wpromote Slack app install (Anthony), bot handle + pilot channel, the other 3 AMs' campaigns.
...
- **AM usage guide:** `docs/qa_buddy_pilot_guide.md` (clean `.docx` rendered via the google-doc skill; older `am_usage_guide.md` retired 2026-06-16). Blanks: #1 template link = **done** (`1rTfqYA3`). #2 AM Slack channel = **pending** (note: the bot's test Slack app lives in a separate "Testing Environment" workspace, so resolve which channel AMs use before sending). #3 feedback channel = **pending** (not set up yet).
...
### CRITICAL pilot gate — the client must be BigQuery-synced (found 2026-06-16)
The hard prerequisite for the bot to work at all is that the client's Meta data is Airbyte-synced into `polaris-data-317717`. **301 clients / 362 accounts** are synced. Screen every pilot AM's `account_id` against `summary.facebook_ads__account_performance` **before** committing them; pilot only synced clients.
- **Kendra Scott** (pilot AM, account `955066737173306`, "Yellow Rose" campaign `120245902610470176`) is **NOT synced** (0 perf rows) — the bot cannot QA it until Airbyte adds it. Avara + Spindrift ARE synced.
- **ZAGG** (pilot AM "INT | ZAGG", account `863237077344182`, ~40 per-campaign tabs in its own checklist format) is **also NOT synced** (0 perf rows). Tally of pilot clients screened so far: Avara + Spindrift synced; Kendra Scott + ZAGG not (about half). The unsynced ones are why tomorrow's marts go-live matters: it likely carries more clients than polaris.
  - **Update 2026-06-16:** Riley announced (Slack 2026-06-11) the data is ready in a NEW project `prj-npd-plrs-tst-marts-onfd` (non-prod "test marts", distinct from the bot's source `polaris-data-317717`; example `C03342659.facebook_ads__adsets`). Likely the migration target with the added fields. **Neither jack.fay nor the bot SA can read it yet (403 for both).** To use it: (1) get BQ read on `prj-npd-plrs-tst-marts-onfd` granted to `jack.fay@wpromote.com` + `ppc-qa-buddy@prj-prd-ai-ppc-qa-pkph.iam.gserviceaccount.com`; (2) repoint the adapter (`BQ_META_PROJECT` + table paths + resolver) off `polaris-data-317717` (a swap, not a rewrite, the adapter is swappable by design) and validate. Once access lands, verify KS presence + whether `promoted_object`/conversion-event + the other gated fields now exist (would auto-upgrade the Review-gated checks).
- **AM-format finding:** AMs use their *own* QA checklists, not our Check_ID/Builder-Input template (KS's is "KS | Paid Social | QA Checklist": per-campaign tabs, manual TRUE/FALSE + an "Output" column holding the expected values). Do NOT ask them to adopt our template. Pull the campaign + expected values from what they already produce, and run the bot for them. The campaign account+IDs live in the sheet's "Link To Campaign" **rich-text hyperlink** — extract with `sh.fetch_sheet_metadata(params={'includeGridData':True,'ranges':..., 'fields':'sheets.data.rowData.values(formattedValue,hyperlink,textFormatRuns.format.link.uri)'})`.
