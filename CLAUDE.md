...
**Build status: vertical slice built, data layer validated against live BigQuery, Gemini wired through orchestration, Sheets auth path matches Maya's prod pattern.** Repo at `github.com/jackfay2/paid-social-qa-buddy` (local: `/Users/jack.fay/paid-social-qa-buddy`). Built + unit-tested (291 tests, CI green on push): five backing-service adapters + a new Secret Manager adapter, account resolver, Gemini text-check adapter wired through orchestration (single batched call per job, per-ad evaluation with row-level aggregation, defaults to Review on any uncertainty), pipeline, orchestration, worker endpoint, Cloud Tasks OIDC auth, 5 campaign-level deterministic checks, 7 ad-set-level deterministic checks (status / start_date / end_date / age_min / age_max / genders / countries — targeting reads handle nested-vs-flat schema variance), 3 ad-level deterministic checks (status / count / destination_url — URL normalizer deliberately strict: any whitespace or dot-less host returns Review, never a false Fix). Sheets auth now mirrors Maya's prod pattern: `QA_SHEETS_AUTH_MODE` ∈ {service_account, adc, auto}; `GOOGLE_SHEETS_SERVICE_ACCOUNT_JSON_SECRET_NAME` resolves through `_resolve_secrets()` at startup; explicit env wins over a configured secret. The **full worker has been run end-to-end against real BigQuery** (resolver + fetch + orchestration + checks → correct verdicts, 2026-05-27). The literal Google Sheets and Slack calls remain mocked in tests; live validation is gated on actually getting the SA JSON into a Secret Manager secret (GCP-provisioning step). See Implementation status below.
...
Built and unit-tested (291 tests; CI runs them on push). The data layer + full orchestration have been validated against live BigQuery. Each adapter sits behind a Protocol in `app/core/contracts.py` (12-factor IV).
...
- Social repo: `source .venv/bin/activate && pytest -q` (291 tests). The venv must be Python 3.11 (Homebrew at `/Users/jack.fay/homebrew`), not the system 3.9.
