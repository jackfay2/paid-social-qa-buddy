...
- **Phase B (next, with Pamela):** add the **trafficking table** `AirTable_v2.wp_live_trafficking` (197 cols: intended specs, QC flags, Offer/Show/Genre, Frame_Size, flight dates, build-time coverage). It does NOT share a clean key with the perf table (Creative_IDs differ; arrays) — links by File_Name/campaign name. **Ask Pamela the canonical join** before building. This adds the bonus Offer/Show/dimensions checks + pre-launch QA.
