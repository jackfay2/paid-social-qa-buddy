...
**Build status: vertical slice built, data layer validated against live BigQuery, Gemini wired through orchestration.** Repo at `github.com/jackfay2/paid-social-qa-buddy` (local: `/Users/jack.fay/paid-social-qa-buddy`). Built + unit-tested (263 tests, CI green on push): five backing-service adapters, account resolver, Gemini text-check adapter wired through orchestration (single batched call per job, per-ad evaluation with row-level aggregation, defaults to Review on any uncertainty), pipeline, orchestration, worker endpoint, Cloud Tasks OIDC auth, 5 campaign-level deterministic checks, 7 ad-set-level deterministic checks (status / start_date / end_date / age_min / age_max / genders / countries — targeting reads handle nested-vs-flat schema variance), 3 ad-level deterministic checks (status / count / destination_url — URL normalizer deliberately strict: any whitespace or dot-less host returns Review, never a false Fix). The **full worker has been run end-to-end against real BigQuery** (resolver + fetch + orchestration + checks → correct verdicts, 2026-05-27). The literal Google Sheets and Slack calls remain mocked/simulated (Sheets OAuth is blocked on this workspace — Maya's prod pattern is SA-JSON-from-Secret-Manager, see Open decisions). See Implementation status below.
